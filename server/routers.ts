...
export const appRouter = router({
  ...
  reminders: router({
    ...
    // Bulk send reminders for selected vehicles
    bulkSendReminders: publicProcedure
      .input(z.object({
        vehicleIds: z.array(z.number()),
      }))
      .mutation(async ({ input }) => {
        const { getVehiclesWithCustomersForReminders, createReminderLog, createReminder, updateReminder } = await import("./db");
        const { sendMOTReminderWithTemplate, sendSMS } = await import("./smsService");
        
        const vehicles = await getVehiclesWithCustomersForReminders();
        const selectedVehicles = vehicles.filter(v => input.vehicleIds.includes(v.vehicleId));
        ...
        for (const vehicle of selectedVehicles) {
          try {
            if (!vehicle.customerPhone) {
              results.failed++;
              results.errors.push(`${vehicle.registration}: No customer phone number`);
              continue;
            }
            
            if (!vehicle.motExpiryDate) {
              results.failed++;
              results.errors.push(`${vehicle.registration}: No MOT expiry date`);
              continue;
            }
            
            const motExpiry = new Date(vehicle.motExpiryDate);
            const today = new Date();
            const daysLeft = Math.ceil((motExpiry.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
            
            // Only send if MOT is expiring within 30 days or already expired
            if (daysLeft > 30) {
              results.failed++;
              results.errors.push(`${vehicle.registration}: MOT not due yet (${daysLeft} days left)`);
              continue;
            }
            ...
            const formattedDate = motExpiry.toLocaleDateString("en-GB");
            let messageContent: string;
            let result: { success: boolean; messageId?: string; error?: string };
            
            if (daysLeft >= 0) {
              messageContent = `Hi ${vehicle.customerName || "Customer"}, this is a reminder that your vehicle ${vehicle.registration} has an MOT due on ${formattedDate} (${daysLeft} days). Please contact us to book your MOT.`;
              result = await sendMOTReminderWithTemplate({
                to: vehicle.customerPhone,
              ...
              messageContent = `Hi ${vehicle.customerName || "Customer"}, your vehicle ${vehicle.registration} MOT expired on ${formattedDate} (${daysOverdue} days ago). Please contact us urgently to book your MOT.`;
              result = await sendSMS({
                to: vehicle.customerPhone,
            ...
            if (!result.success) {
              results.failed++;
              results.errors.push(`${vehicle.registration}: ${result.error || "Failed to send"}`);
              continue;
            }
            
            // Update reminder status
            const reminderId = Number((reminderResult as any).insertId);
            ...
            await createReminderLog({
              ...
              vehicleId: vehicle.vehicleId,
              messageType: "MOT",
              recipient: vehicle.customerPhone,
              messageSid: result.messageId || "",
              status: "sent",
              templateUsed: daysLeft >= 0 ? "motreminder" : "freeform",
              customerName: vehicle.customerName,
              registration: vehicle.registration,
              dueDate: motExpiry,
              messageContent: messageContent,
            ...
            results.failed++;
            results.errors.push(`${vehicle.registration}: ${error.message}`);
          }
        }
