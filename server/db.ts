...
export async function getAllVehiclesWithCustomers() {
  ...
  try {
    ...
    const followUpCalls = await db
      ...
      .from(customerLogs)
      .where(and(eq(customerLogs.type, "call"), ilike(customerLogs.subject, "MOT follow-up%"),
        isNotNull(customerLogs.vehicleId), gte(customerLogs.createdAt, oneYearAgo)));
...
export async function getOmnipartOrderMeta(refs?: string[]) {
  const db = await getDb();
  const out = new Map<string, { category: string | null; partStates: Record<string, StoredUsage>; jobSheetId: number | null }>();
  if (!db) return out;
