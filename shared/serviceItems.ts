 ...
 */
export type ServiceItem = { key: string; label: string; group: "Service" | "Fluids" | "Filters" | "Wear" | "Other"; test: RegExp };

export const SERVICE_ITEMS: ServiceItem[] = [
  // Whole services first — a "major service" implies the oil and filters under it.
  { key: "majorService", label: "Major service", group: "Service", test: /\b(major|full)\s+service\b/ },
  { key: "smallService", label: "Small service", group: "Service", test: /\b(small|interim|basic|minor)\s+service\b/ },
  { key: "service", label: "Service", group: "Service", test: /\bservice\b(?!\s*(check|charge|centre))/ },

  { key: "cambelt", label: "Cambelt / timing belt", group: "Wear", test: /\b(cam\s?belt|timing\s?belt|timing\s?chain|water\s?pump\s*(&|and)?\s*timing)\b/ },
  { key: "auxBelt", label: "Aux / drive belt", group: "Wear", test: /\b(aux(iliary)?\s?belt|drive\s?belt|fan\s?belt|serpentine)\b/ },

  { key: "brakeFluid", label: "Brake fluid", group: "Fluids", test: /brake\s*fluid/ },
  { key: "coolant", label: "Coolant / antifreeze", group: "Fluids", test: /\b(anti[-\s]?freeze|coolant)\b/ },
  { key: "gearboxOil", label: "Gearbox / transmission oil", group: "Fluids", test: /\b(gearbox|transmission|diff(erential)?)\s*(oil|fluid)\b/ },
  ...
  { key: "engineOil", label: "Engine oil", group: "Fluids", test: /\b(engine\s*oil|\d{1,2}w[\s/-]?\d{2}|castrol|fully\s*synth)/ },

  { key: "oilFilter", label: "Oil filter", group: "Filters", test: /oil\s*filter/ },
  { key: "airFilter", label: "Air filter", group: "Filters", test: /air\s*filter/ },
  { key: "fuelFilter", label: "Fuel filter", group: "Filters", test: /(fuel|diesel)\s*filter/ },
  { key: "pollenFilter", label: "Pollen / cabin filter", group: "Filters", test: /(pollen|cabin|interior)\s*filter/ },

  { key: "sparkPlugs", label: "Spark plugs", group: "Wear", test: /spark\s*plug|glow\s*plug/ },
